...
function Remove-WinGetApp {
    ...
    if ($script:Params.ContainsKey("User")) {
        Write-Host "正在添加计划任务以为用户 $(GetUserName) 卸载 $app..."
        Set-RunOnceWingetTask -appId $app
    }
    elseif ($script:Params.ContainsKey("Sysprep")) {
        Write-Host "正在添加计划任务以为新用户卸载 $app..."
        Set-RunOnceWingetTask -appId $app
    }
}
...
function Get-AppRemovalMethod {
    ...
    if (-not $script:AppRemovalMethodCache) {
        ...
        try {
            ...
            Write-Warning "无法从 '$script:AppsListFilePath' 加载应用移除方法。未知应用将默认使用 Appx。错误：$_"
        }
    }
...
function Set-RunOnceWingetTask {
    ...
    try {
        ...
        Write-Host "为 $($appId) 安排卸载任务失败：$_" -ForegroundColor Red
    }
}
