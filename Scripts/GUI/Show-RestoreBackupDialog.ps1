...
function Show-RestoreBackupDialog {
    ...
    $showStartMenuOverviewState = {
        ...
        $overviewSummaryText.Text = "这将使用所选备份替换 $($scopeInfo.SummaryText) 当前固定的开始菜单应用布局。"
    ...
    $showRegistryOverview = {
        ...
        Write-Host "备份概览已准备。将还原=$($revertibleFeaturesList.Count)，将重新应用=$($reappliedFeaturesList.Count)，不可还原=$($nonRevertibleFeaturesList.Count)"
    ...
    try {
        ...
        throw "显示还原备份对话框失败。错误：$($_.Exception.Message) 内部：$innerMessage"
    }
    finally {
