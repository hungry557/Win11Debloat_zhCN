...
function DisableStoreSearchSuggestions {
    ...
    if ($script:Params.ContainsKey("WhatIf")) {
        Write-Host "[WhatIf] 通过限制对 ${StoreAppsDatabase} 的访问来为用户 $userName 禁用 Microsoft Store 搜索建议" -ForegroundColor Cyan
        return
    }
    ...
    {
        Write-Host "找不到用户 $userName 的 Store 应用数据库，正在创建以防止 Windows 稍后创建..." -ForegroundColor Yellow
...
function EnableStoreSearchSuggestions {
    ...
    if ($script:Params.ContainsKey("WhatIf")) {
        Write-Host "[WhatIf] 通过恢复对 ${StoreAppsDatabase} 的访问来为用户 $userName 重新启用 Microsoft Store 搜索建议" -ForegroundColor Cyan
        return
    }
